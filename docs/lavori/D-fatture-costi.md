...
| | |
|---|---|
| Tabelle | `fatture` · `fornitori`, `mezzi`, `costi` |
| Viste da cui leggere | `v_fatture` — aggiunge `incassato`, `residuo`, `stato_effettivo`<br>`v_costi` — aggiunge fornitore, targa e numero commessa |
| File di schema | `db/004_fatture.sql`, `db/005_costi.sql` |
| Seed da scrivere | `db/013_seed_fatture.sql`, `db/014_seed_costi.sql` |

**`fatture.stato` accetta solo `bozza` ed `emessa`.** `pagata_parziale`, `pagata` e `scaduta`
non sono valori salvabili: li calcola `v_fatture` da `incassi` e `data_scadenza`. È la regola
centrale che avevi già, resa non aggirabile da un `CHECK`. Registrare un incasso vuol dire
aggiornare la colonna JSONB `incassi`: lo stato si aggiorna da sé.

`v_fatture` ti dà anche `incassato` e `residuo` già sommati — non ricalcolarli nel frontend.

Sui costi il vincolo che conta è **`chk_carburante_ha_mezzo`**: una riga con
`categoria = 'carburante'` e `mezzo_id` nullo viene rifiutata dal database. È il requisito
«carburante distinto per mezzo» reso non aggirabile, e vale anche per gli import. Stessa cosa
per `chk_noleggio_ha_tipo`.

`mezzi.targa` è `UNIQUE`: un secondo inserimento con la stessa targa dà un errore 23505, da
mostrare come «Esiste già un mezzo con questa targa» e non come errore generico.
